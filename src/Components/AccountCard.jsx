...
const AccountCard = ({ member, children, recommended, href }) => {
  const card = (
    <article
    ...
    >
      ...
      <div>
        ...
        <p
          className={`max-w-40 text-small-text-b font-light leading-4 ${member ? "text-slate-600" : "text-blue-100"}`}
        >
          {member
            ? "Set up or access Digital Banking"
            : "Become a member and open an account"}
        </p>
      </div>
  ...
  return href ? (
    <a href={href} className="block w-full lg:max-w-80">
      {card}
    </a>
  ) : (
    card
